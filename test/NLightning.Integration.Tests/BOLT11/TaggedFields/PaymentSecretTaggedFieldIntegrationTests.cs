...
public class PaymentSecretTaggedFieldIntegrationTests
{
    private static readonly uint256 s_knownSecret = new(
        new byte[]
        {
    ...
    private static (byte[] Buffer, int FieldOffsetBits) BuildBuffer(int prePadBits, int postPadBits)
    {
        ...
        var field = new PaymentSecretTaggedField(s_knownSecret);
    ...
    public void FromBitReader_Reads_From_Beginning_Of_Buffer()
    {
        ...
        // Act
        var parsed = PaymentSecretTaggedField.FromBitReader(reader, TaggedFieldConstants.HashLength);

        // Assert
        Assert.Equal(s_knownSecret, parsed.Value);
    }
