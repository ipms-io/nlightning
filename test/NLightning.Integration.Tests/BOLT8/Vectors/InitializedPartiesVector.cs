...
internal sealed class InitializedPartiesVector
{
    ...
    public InitializedPartiesVector()
    {
        var initiator = new HandshakeState(true, InitiatorValidKeysVector.LocalStaticPrivateKey,
                                           ...
                                           new FakeFixedKeyDh(InitiatorValidKeysVector.EphemeralPrivateKey));
        var responder = new HandshakeState(false, ResponderValidKeysVector.LocalStaticPrivateKey,
                                           ...
                                           new FakeFixedKeyDh(ResponderValidKeysVector.EphemeralPrivateKey));
        ...
        (initiatorMessageSize, _, initiatorTransport) =
        ...
        // Get sk
        var c1 =
            ((CipherState?)InitiatorTransport.GetType().GetField("_sendingKey", flags)?.GetValue(InitiatorTransport) ??
             throw new MissingFieldException("_sendingKey")) ?? throw new NullReferenceException("_sendingKey");
        InitiatorSk =
            ((SecureMemory?)c1.GetType().GetField("_k", flags)?.GetValue(c1) ??
             throw new MissingFieldException("_sendingKey._k")) ?? throw new NullReferenceException("_sendingKey._k");
        // Get rk
        var c2 =
            ((CipherState?)InitiatorTransport.GetType().GetField("_receivingKey", flags)
                                            ?.GetValue(InitiatorTransport) ??
             throw new MissingFieldException("_receivingKey")) ?? throw new NullReferenceException("_receivingKey");
        InitiatorRk =
