...
public class ResponderIntegrationTests
{
    ...
    {
        // Arrange
        var responder = new HandshakeState(false, ResponderValidKeysUtil.LocalStaticPrivateKey,
                                           ResponderValidKeysUtil.LocalStaticPublicKey, s_dhFake);
        ...
        {
            // Act
            var (messageSize, handshakeHash, transport) = responder.ReadMessage(ResponderValidKeysUtil.ActOneInput,
                messageBuffer);
    ...
    public void Given_ValidActTwoMessage_When_ResponderReadsActThree_Then_ItDoesntThrow()
    {
        ...
        try
        {
            ...
            // Act
            var (messageSize, handshakeHash, transport) = responder.ReadMessage(ResponderValidKeysUtil.ActThreeInput,
                messageBuffer);
            var message = messageBuffer[..messageSize];
    ...
    public void Given_ValidActTwoRead_When_InitiatorWritesActThree_Then_KeysShouldBeValid()
    {
        ...
        try
        {
            ...
            Assert.Equal(((Span<byte>)ResponderValidKeysUtil.OutputRk).ToArray(), ((Span<byte>)rk).ToArray());
            Assert.Equal(((Span<byte>)ResponderValidKeysUtil.OutputSk).ToArray(), ((Span<byte>)sk).ToArray());
        }
        finally
        {
            responder.Dispose();
