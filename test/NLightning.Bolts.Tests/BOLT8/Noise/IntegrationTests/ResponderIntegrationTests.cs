...
{
    private static readonly ResponderValidKeys _validKeys = new();
    ...
    public void Given_ValidKeys_When_ResponderReadsActOne_Then_ItDoesntThrow()
    {
        ...
        setDh.Invoke(responder, [new FixedKeyDh(_validKeys.EphemeralPrivateKey)]);
    ...
    public void Given_ValidActTwoRead_When_InitiatorWritesActThree_Then_KeysShouldBeValid()
    {
        ...
        var rk = ((byte[]?)c1.GetType().GetField("_k", flags)?.GetValue(c1) ?? throw new MissingFieldException("_c1._k")) ?? throw new ArgumentNullException("_c1._k");
        // Get sk
        ...
        var sk = ((byte[]?)c2.GetType().GetField("_k", flags)?.GetValue(c2) ?? throw new MissingFieldException("_c2._k")) ?? throw new ArgumentNullException("_c2._k");
