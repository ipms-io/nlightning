...
// 	[Fact]
// 	public void TestCacophony() => Test("BOLT8/Noise/Vectors/cacophony.txt");

// 	[Fact]
// 	public void TestMultipsk() => Test("BOLT8/Noise/Vectors/snow-multipsk.txt");
...
// 			var setDh = init.GetType().GetMethod("SetDh", flags) ?? throw new MissingMethodException("SetDh");

// 			setDh.Invoke(init, [new FixedKeyDh(DhFunction.CURVE25519, initEphemeral)]);
// 			setDh.Invoke(resp, [new FixedKeyDh(DhFunction.CURVE25519, respEphemeral)]);
...
// 	{
// 		var s = File.ReadAllText("BOLT8/Noise/Vectors/noise-c-fallback.txt");
// 		var json = JObject.Parse(s);
...
// 	{
// 		return token[property]?.Select(psk => Hex.Decode(psk?.ToString() ?? string.Empty)).ToList() ?? [];
// 	}
