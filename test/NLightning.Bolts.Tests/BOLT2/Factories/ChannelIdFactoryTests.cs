...
public class ChannelIdFactoryTests
{
    ...
    public void Given_ValidInputs_When_CreatingV1_Then_ReturnsCorrectChannelId()
    {
        ...
        ushort fundingOutputIndex = 1;

        // Act
        var channelId = ChannelIdFactory.CreateV1(fundingTxId, fundingOutputIndex);

        // Assert
        var expectedChannelId = new byte[32];
        fundingTxId.CopyTo(expectedChannelId, 0);
        expectedChannelId[30] ^= (byte)(fundingOutputIndex >> 8);
        expectedChannelId[31] ^= (byte)(fundingOutputIndex & 0xFF);
    ...
    public void Given_InvalidFundingTxIdLength_When_CreatingV1_Then_ThrowsArgumentException()
    {
        ...
        // Act & Assert
        var ex = Assert.Throws<ArgumentException>(() => ChannelIdFactory.CreateV1(fundingTxId, fundingOutputIndex));
