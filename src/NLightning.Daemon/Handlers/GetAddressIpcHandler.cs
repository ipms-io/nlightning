...
public class GetAddressIpcHandler : IIpcCommandHandler
{
    private readonly ILogger<GetAddressIpcHandler> _logger;
    private readonly IServiceProvider _serviceProvider;

    public NodeIpcCommand Command => NodeIpcCommand.GetAddress;
    ...
    public async Task<IpcEnvelope> HandleAsync(IpcEnvelope envelope, CancellationToken ct)
    {
        try
        {
            ...
            if (request.AddressType.HasFlag(AddressType.P2Tr))
                p2Tr = await walletAddressService.GetUnusedAddressAsync(AddressType.P2Tr, false);

            if (request.AddressType.HasFlag(AddressType.P2Wpkh))
                p2Wpkh = await walletAddressService.GetUnusedAddressAsync(AddressType.P2Wpkh, false);
