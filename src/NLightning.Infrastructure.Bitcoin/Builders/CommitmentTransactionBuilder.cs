...
public class CommitmentTransactionBuilder : ICommitmentTransactionBuilder
{
    ...
    public CommitmentTransactionBuilder(Network network)
    {
        _network = network;
    }
    ...
    {
        if (transaction.FundingOutput.TxId is null || transaction.FundingOutput.Index is null)
        ...
        var outpoint = new OutPoint(new uint256(transaction.FundingOutput.TxId), transaction.FundingOutput.Index.Value);
